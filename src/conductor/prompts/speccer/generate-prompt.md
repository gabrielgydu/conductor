...
5. **Frontend Architecture** — Routes, stores, components from frontend domain specs
...
3. **Domain → Phase Mapping Rules:**
   ...
   - Within a priority tier, use dependency order:
     ...
     - Frontend views → Phase 4+ (one phase per major view, or group small views)
     - Background Jobs → wherever they fit naturally in the dependency chain
...
## Final Verification
1. Run `{test command}` — ALL tests must pass
2. Run `{lint command}` — must show no errors
