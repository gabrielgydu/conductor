...
def _progress_file_path(state: ConductorState, run_idx: int, stage_idx: int) -> Path | None:
...
def create_worktree(
    ...
    *,
    ...
    subprocess.run(
        ["git", "worktree", "remove", "--force", str(wt_candidate)],
    ...
    if wt_candidate.exists():
        shutil.rmtree(str(wt_candidate), ignore_errors=True)
    ...
    _log(
        ...
        log_path, audit_path,
        run=run_idx, stage=stage_idx, branch=branch,
        worktree=str(wt_candidate), base_ref=base_ref,
    )
...
async def run_speccer_init(
    ...
    if wiring is not None:
        ...
        if wiring_type == "external" and source_path:
            if not Path(source_path).exists():
                _log("SPECCER_INVOKE", f"Warning: external context path not found: {source_path}",
                     log_path, audit_path)
            ...
            if source_run_i in run_by_index:
                ...
                if not Path(full_ctx).exists():
                    _log("SPECCER_INVOKE", f"Warning: context wiring source not found: {full_ctx}",
                         log_path, audit_path)
    ...
    _log("SPECCER_INVOKE", f"speccer init {fname} (mode={mode})", log_path, audit_path,
         run=run_idx, stage=stage_idx, command="speccer init", mode=mode)
...
async def run_speccer_run(
    ...
    _log("SPECCER_INVOKE", f"speccer run {fname}", log_path, audit_path,
         run=run_idx, stage=stage_idx, command="speccer run")
...
async def run_speccer_continue(
    ...
    cmd = f"cd {wt} && {_SPECCER_BIN} run --feature {fname} --project-dir {wt} --continue"
    await tmux.spawn_in_window_and_wait(window_name, cmd, exit_file=exit_file, cwd=wt)

    _log("SPECCER_INVOKE", f"speccer run --continue {fname}", log_path, audit_path,
         run=run_idx, stage=stage_idx, command="speccer run --continue")
...
async def run_speccer_generate(
    ...
    _log("SPECCER_INVOKE", f"speccer generate {fname}", log_path, audit_path,
         run=run_idx, stage=stage_idx, command="speccer generate")
...
def sync_speccer_status(
    ...
    if speccer_status in status_map:
        ...
        _log("SPECCER_STATUS_SYNC",
             f"Unknown speccer status: {speccer_status}",
             log_path, audit_path,
             run=run_idx, stage=stage_idx, speccer_status=speccer_status)
        stage.status = StageStatus.FAILED
...
def pre_reset_speccer_status(
    ...
    if speccer_status == "EXPLORING":
        ...
        content = re.sub(r"^STATUS:.*", "STATUS: NEEDS_INPUT", content, flags=re.MULTILINE)
        progress_file.write_text(content, encoding="utf-8")
    ...
    if exit_code == 0:
        ...
        if progress_file is not None and progress_file.exists():
            ...
            if pstatus in _recoverable:
                sync_speccer_status(state, run_idx, stage_idx, storage, log_path, audit_path)
                return
        _log("FAILURE", f"Speccer exited {exit_code} for {fname}", log_path, audit_path,
             run=run_idx, stage=stage_idx, exit_code=exit_code)
        stage.status = StageStatus.FAILED
    else:
...
def write_feature_description(
    ...
    if not source.exists():
        _log("FILE_WRITE", f"Warning: description file missing: {desc_file_rel}",
             log_path, audit_path)
        return
    ...
    _log("FILE_WRITE", f"Feature description for {fname} ({dest.stat().st_size} bytes)",
         log_path, audit_path,
         run=run_idx, stage=stage_idx, file="FEATURE-DESCRIPTION.md")
...
def write_constitution(
    ...
    _log("FILE_WRITE", f"Constitution for {fname}", log_path, audit_path,
         run=run_idx, stage=stage_idx, file="CONSTITUTION.md")
...
async def answer_questions(
    ...
    _log("BRAIN_CALL", f"answer-questions for {fname}", log_path, audit_path,
         type="answer-questions", run=run_idx, stage=stage_idx)
...
async def start_runner(
    ...
    """Spawn runner in tmux window (non-blocking). Writes exit code to /tmp file."""
    run = state.runs[run_idx]
    stage = run.stages[stage_idx]
    fname = run.name + stage.feature_suffix
    wt = stage.worktree
    ...
    cmd = f"cd {wt} && bash docs/{fname}/run.sh --steerable"
    await tmux.spawn_runner_in_window(window_name, cmd, exit_file=exit_file, cwd=wt)

    stage.started_at = datetime.now(timezone.utc)

    _log("RUNNER_START", f"Runner for {fname} [window: {window_name}]", log_path, audit_path,
         run=run_idx, stage=stage_idx, worktree=wt, tmux_window=window_name)
...
async def check_stall(
    ...
    _log("STALL_CHECK",
         ...
         log_path, audit_path,
         run=run_idx, stage=stage_idx,
         progress_hash=current_hash, stall_count=stall_count, threshold=2)
    ...
    if stall_count >= 5:
        ...
        _log("STALL_CHECK",
             ...
             log_path, audit_path,
             run=run_idx, stage=stage_idx, stall_count=stall_count)
        return
    ...
    if stall_count >= 2:
        ...
        if stage.status == StageStatus.EXECUTING:
            ...
            matches = sorted(_glob.glob(pattern), key=lambda p: Path(p).stat().st_mtime, reverse=True)
            if matches:
                lines = Path(matches[0]).read_text(encoding="utf-8", errors="replace").splitlines()
                context_parts.append("## Activity Log (last 100 lines)\n" + "\n".join(lines[-100:]))
        else:
            progress_file = _progress_file_path(state, run_idx, stage_idx)
            if progress_file is not None and progress_file.exists():
                context_parts.append("## PROGRESS.md\n" + progress_file.read_text(encoding="utf-8"))

        import json as _json
        context_parts.append("## Stage Config\n" + _json.dumps(
            stage.model_dump(), indent=2, default=str
        ))
        ...
        _log("BRAIN_CALL", f"diagnose-stall for {fname}", log_path, audit_path,
             type="diagnose-stall", run=run_idx, stage=stage_idx)
        ...
        if action == "STEER":
            ...
            lines_after = response_text.split("ACTION: STEER", 1)[-1].strip().splitlines()[:5]
            message = "\n".join(lines_after)
            if message and stage.status == StageStatus.EXECUTING:
                ...
                try:
                    ...
                    _log("RUNNER_STEER", f"Steered {fname}", log_path, audit_path,
                         run=run_idx, stage=stage_idx)
                except Exception:
                    pass
        elif action == "RESET":
...
async def monitor_runner(
    ...
    if exit_file.exists():
        ...
        if exit_code == 0:
            ...
            _log("RUNNER_EXIT", f"Runner {fname} exit 0", log_path, audit_path,
                 run=run_idx, stage=stage_idx, exit_code=0)
        else:
            stage.status = StageStatus.FAILED
            _log("RUNNER_EXIT", f"Runner {fname} exit {exit_code}", log_path, audit_path,
                 run=run_idx, stage=stage_idx, exit_code=exit_code)
    else:
        ...
        if is_idle:
            stage.status = StageStatus.FAILED
            _log("RUNNER_DEAD", f"Runner {fname} died without exit file (window idle)",
                 log_path, audit_path,
                 run=run_idx, stage=stage_idx, window=window_name)
        else:
            await check_stall(state, run_idx, stage_idx, tmux, storage, config, log_path, audit_path)
...
async def handle_failure(
    ...
    if retries >= 2:
        ...
        _log("BLOCKED",
             ...
             log_path, audit_path,
             run=run_idx, stage=stage_idx, reason="max_retries")
        return
    ...
    _log("BRAIN_CALL", f"diagnose-failure for {fname}", log_path, audit_path,
         type="diagnose-failure", run=run_idx, stage=stage_idx)
    ...
    _log("FAILURE",
         ...
         log_path, audit_path,
         run=run_idx, stage=stage_idx, retries=retries, max_retries=2, action=action)

    if action == "RETRY":
        stage.retries += 1
        run.monitor.retry_count += 1
        await restart_stage(state, run_idx, stage_idx, storage,
...
async def restart_stage(
    ...
    if current_status in (StageStatus.SPEC_INIT, StageStatus.SPEC_RUNNING,
                          StageStatus.SPEC_NEEDS_INPUT, StageStatus.FAILED):
    ...
    _log("RETRY",
         ...
         log_path, audit_path,
         run=run_idx, stage=stage_idx, retry_count=stage.retries)
...
def push_and_create_pr(
    ...
    if not branch:
        _log("GIT_PUSH_FAILED", f"No branch found for run {run_idx}",
             log_path, audit_path, run=run_idx, reason="no_branch")
        return
    ...
    if push_result.returncode != 0:
        _log("GIT_PUSH_FAILED",
             f"Push failed for {branch}: {push_result.stderr.strip()}",
             log_path, audit_path, run=run_idx, branch=branch)
        return

    _log("GIT_PUSH", f"Pushed {branch}", log_path, audit_path, run=run_idx, branch=branch)
    ...
    if slug_result.returncode != 0:
        _log("PR_CREATE_FAILED", "Could not determine repo slug",
             log_path, audit_path, run=run_idx, reason="no_repo_slug")
        return
    ...
    if pr_result.returncode == 0:
        ...
        run.pr_url = pr_url
        _log("PR_CREATED", pr_url, log_path, audit_path,
             run=run_idx, branch=branch, base=base_branch, pr_url=pr_url)
    else:
        _log("PR_CREATE_FAILED",
             f"gh pr create failed: {pr_result.stdout.strip()} {pr_result.stderr.strip()}",
             log_path, audit_path, run=run_idx, branch=branch, base=base_branch)
...
def _has_foreground_runs(state: ConductorState, active_run_indices: list[int]) -> bool:
    ...
    foreground = {
        StageStatus.PENDING, StageStatus.SPEC_INIT, StageStatus.SPEC_RUNNING,
        StageStatus.SPEC_NEEDS_INPUT, StageStatus.SPEC_COMPLETE, StageStatus.GENERATED,
    }
    run_by_index = {r.index: r for r in state.runs}
...
async def advance_run(
    ...
    if stage_idx >= num_stages:
        ...
        _log("RUN_COMPLETE", f"Run {run.name} complete", log_path, audit_path,
             run=run_idx, name=run.name)
    ...
    if status == StageStatus.PENDING:
        create_worktree(state, run_idx, stage_idx, storage, log_path=log_path, audit_path=audit_path)  # noqa: E501
        await run_speccer_init(state, run_idx, stage_idx, tmux, storage, log_path, audit_path)
        write_feature_description(state, run_idx, stage_idx, storage,
                                  log_path=log_path, audit_path=audit_path)
    ...
    if status in (StageStatus.SPEC_INIT, StageStatus.SPEC_RUNNING):
        pre_reset_speccer_status(state, run_idx, stage_idx, storage)
        await run_speccer_run(state, run_idx, stage_idx, tmux, storage, log_path, audit_path)
        fname = run.name + stage.feature_suffix
        speccer_exit_code_handler(state, run_idx, stage_idx, fname, storage, log_path, audit_path)
        atomic_save(state, storage.conductor_state(state.project_name))
        return
    ...
    if status == StageStatus.SPEC_NEEDS_INPUT:
        ...
        if overnight:
            await answer_questions(state, run_idx, stage_idx, storage,
                                   log_path=log_path, audit_path=audit_path)
            await run_speccer_continue(state, run_idx, stage_idx, tmux, storage, log_path, audit_path)
            fname = run.name + stage.feature_suffix
            speccer_exit_code_handler(state, run_idx, stage_idx, fname, storage, log_path, audit_path)
            atomic_save(state, storage.conductor_state(state.project_name))
    ...
    if status == StageStatus.SPEC_COMPLETE:
        await run_speccer_generate(state, run_idx, stage_idx, tmux, storage, log_path, audit_path)
        stage.status = StageStatus.GENERATED
        atomic_save(state, storage.conductor_state(state.project_name))
        return
    ...
    if status == StageStatus.GENERATED:
        await start_runner(state, run_idx, stage_idx, tmux, storage, log_path, audit_path)
    ...
    if status == StageStatus.EXECUTING:
        await monitor_runner(state, run_idx, stage_idx, tmux, storage, config, log_path, audit_path)
        atomic_save(state, storage.conductor_state(state.project_name))
        return
    ...
    if status == StageStatus.DONE:
        ...
        if next_stage < num_stages:
            ...
            _log("STAGE_TRANSITION",
                 f"Run {run.name}: {from_name} -> {to_name}",
                 log_path, audit_path,
                 run=run_idx, from_stage=stage_idx, from_name=from_name,
                 to_stage=next_stage, to_name=to_name)
        atomic_save(state, storage.conductor_state(state.project_name))
        return

    # ----- stalled | failed -----
    if status in (StageStatus.STALLED, StageStatus.FAILED):
        await handle_failure(state, run_idx, stage_idx, storage, config, log_path, audit_path)
        atomic_save(state, storage.conductor_state(state.project_name))
        return
...
async def conductor_post_run(
    ...
    _log("POST_RUN_START", f"Starting post-run processing (reason: {exit_reason})",
         log_path, audit_path, reason=exit_reason)

    try:
        await _review_learnings(state, storage, log_path, audit_path)
    except Exception as exc:
        _log("POST_RUN", f"Learnings review failed (non-fatal): {exc}", log_path, audit_path)

    try:
        await _generate_audit_report(state, storage, log_path, audit_path)
    except Exception as exc:
        _log("POST_RUN", f"Audit report generation failed (non-fatal): {exc}", log_path, audit_path)
...
async def _review_learnings(
    ...
    claude_md_files = _glob.glob(
        f"{project_dir}/**/.claude/CLAUDE.md", recursive=True
    )
    claude_md_files += _glob.glob(f"{project_dir}/.claude/CLAUDE.md")
    claude_md_files = [f for f in claude_md_files if "node_modules" not in f and "/.git/" not in f]

    if not claude_md_files:
        _log("POST_RUN", "No CLAUDE.md files found in project — skipping review",
             log_path, audit_path)
        return
    ...
    for f in claude_md_files:
        ...
        claudemd_parts.append(f"### FILE: {rel}\n{Path(f).read_text(encoding='utf-8')}")
        ...
        if content_match:
            ...
            if addition:
                with open(str(full_path), "a", encoding="utf-8") as fh:
                    fh.write(f"\n## Conductor Learnings\n\n{addition}\n")
                updates_applied += 1
                _log("POST_RUN", f"Updated {file_path} with learnings",
                     log_path, audit_path, file=file_path)

    _log("POST_RUN",
         f"Learnings review complete — {updates_applied} file(s) updated",
         log_path, audit_path, updates_applied=updates_applied)
...
async def _generate_audit_report(
    ...
    summary = {
        ...
        "runs": [
            {
                "name": r.name, "status": str(r.status), "current_stage": r.current_stage,
                "stages": [{"name": s.name, "status": str(s.status), "retries": s.retries}
                            for s in r.stages]
            }
            for r in state.runs
        ],
    ...
    if log_path and log_path.exists():
        ...
        context_parts.append("## Conductor Log (last 100 lines)\n" + "\n".join(lines[-100:]))
    ...
    _log("POST_RUN", f"Audit report written to {report_file}", log_path, audit_path,
...
async def conductor_run_loop(
    ...
    def _signal_handler(signum, frame):
        _log("CONDUCTOR_EXIT", "Interrupted — state saved. Re-run to resume.",
             log_path, audit_path, reason="signal", exit_code=130)
        sys.exit(130)
    ...
    for _iteration in range(config.max_iterations):
        ...
        all_terminal = all(
            r.status in (RunStatus.DONE, RunStatus.BLOCKED)
            for r in state.runs
        )
        ...
        if all_terminal and done_count >= 2 and state.integration is None:
            try:
                ...
                atomic_save(state, storage.conductor_state(state.project_name))
                _log("CONDUCTOR_EXIT", f"Integration merge status={state.integration.status}",
                     log_path, audit_path)
            except Exception as exc:
                from conductor.core.models import IntegrationState
                state.integration = IntegrationState(status="failed", branch="")
                atomic_save(state, storage.conductor_state(state.project_name))
                _log("CONDUCTOR_EXIT", f"Integration merge failed: {exc}",
                     log_path, audit_path)
            exit_reason = "all_done"
            break

        if all_terminal:
            _log("CONDUCTOR_EXIT", "All runs complete", log_path, audit_path,
                 reason="all_done", exit_code=0)
            exit_reason = "all_done"
            break

        if not active_run_indices:
            _log("CONDUCTOR_EXIT", "All remaining runs blocked or waiting on deps",
                 log_path, audit_path, reason="blocked_or_waiting", exit_code=0)
        ...
        for run_idx in active_run_indices:
            ...
            if si < len(run.stages) and run.stages[si].status == StageStatus.EXECUTING:
                any_executing = True
            await advance_run(state, run_idx, tmux, storage, config, log_path, audit_path)
